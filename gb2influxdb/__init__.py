...
class gb2influxdb():
    def __init__(self, uri, username, password, database, force, oneshot):
        self._uri = uri
        ...
        # print(self._db_schema[GADGETBRIDGE_DB_TABLE])
    ...
    def setup(self):
        ...
        # result = self._do_get("SELECT * FROM {} ORDER BY time".format(GADGETBRIDGE_DB_TABLE)).json()
        # print(result)
    ...
    def loop_forever(self):
        # Watch for a change on the DB file. Wait a few seconds. Reload it.
        pass
        # result = self._do_post("CREATE DATABASE gadgetbridge").json()
        # result = self._do_post("DROP DATABASE gadgetbridge").json()
        # print(result)
...
def main(uri, username, password, database, force, oneshot):
    ...
    i = gb2influxdb(uri, username, password, database, force, oneshot)
    i.setup()
    i.loop_forever()
